...
for stock in daily_price['code'].unique():
    ...
    # 导入数据
    datafeed = bt.feeds.PandasData(dataname=data_, fromdate=datetime.datetime(2022, 1, 1),
...
class TestStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        for stock in long_list:
            ...
            data = self.getdatabyname(stock)
            order = self.buy(data=data, size=100)  # 为减少可用资金不足的情况，留 5% 的现金做备用
